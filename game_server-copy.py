...
async def consumer(message, websocket):
    ...
    if received_data['response_type'] == 'join':
        ...
        elif received_data['response_type'] == 'test':
            new_test_data = data_handler.create_output('server', 'test', random.randint(1, 1000))
            await players[received_data['username']].ws.send(data_handler.dict2json(new_test_data))
            data_handler.showdata_out(new_test_data)
